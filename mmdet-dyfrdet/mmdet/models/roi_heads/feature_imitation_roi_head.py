...
from mmdet.core import bbox2result, uncbbox2result, bbox2roi, build_assigner, build_sampler
from ..builder import HEADS, build_head, build_roi_extractor
from .base_roi_head import BaseRoIHead
from .test_mixins import BBoxTestMixin, UncBBoxTestMixin, MaskTestMixin
...
class FIRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward_train_wo_fi(self, x, assign_results, sampling_results,
                            gt_bboxes, gt_labels, img_metas):
    ...
    def _bbox_forward_train(self, x, assign_results, sampling_results,
        ...
        loss_bbox = self.bbox_head.loss(bbox_results['cls_score'],
                                        bbox_results['bbox_pred'], rois,
                                        *bbox_targets)
        if loss_bbox['loss_cls'].isnan().any():
            print("e1")
        if loss_bbox['loss_bbox'].isnan().any():
            print("e2")
...
@HEADS.register_module()
class FIRoIUncHead(BaseRoIHead, UncBBoxTestMixin, MaskTestMixin):
    """Simplest base roi head including one bbox head and one mask head."""
    def __init__(self,
                 roi_size=7,
                 num_gpus=1,
                 ...
                 *args,
                 **kwargs):
        super(FIRoIUncHead, self).__init__(
            *args, init_cfg=init_cfg, **kwargs)
        self.roi_size = roi_size
        self.num_gpus = num_gpus
        self.num_con_queue = num_con_queue
    ...
    def _bbox_forward(self, x, rois):
        ...
        weights = feat_weights.view(_, c, 1, 1).repeat(1, 1, h, w) + 1
        bbox_feats = bbox_feats * weights
        cls_score, bbox_pred_mu, bbox_pred_var, x_reg = self.bbox_head(bbox_feats)

        bbox_results = dict(
            cls_score=cls_score, bbox_pred_mu=bbox_pred_mu, bbox_pred_var=bbox_pred_var, bbox_feats=bbox_feats, comp_feats=comp_feats, x_reg = x_reg)
        return bbox_results
    ...
    def _bbox_forward_train_wo_fi(self, x, assign_results, sampling_results,
        ...
        if loss_bbox['loss_cls'].isnan().any():
            print("e1")
        if loss_bbox['loss_cls'].isinf().any():
            print("e1")
        if loss_bbox['loss_bbox'].isnan().any():
            print("e2")
        if loss_bbox['loss_bbox'].isinf().any():
            print("e3")
        bbox_results.update(loss_bbox=loss_bbox)
        return bbox_results
