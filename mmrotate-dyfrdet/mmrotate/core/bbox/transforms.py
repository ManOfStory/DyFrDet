...
def uncrbbox2result(bboxes, labels, uncertainties, num_classes):
    """Convert detection results to a list of numpy arrays.

    Args:
        bboxes (torch.Tensor): shape (n, 6)
        labels (torch.Tensor): shape (n, )
        uncertainties (torch.Tensor): shape (n, 5)
        num_classes (int): class number, including background class
    ...
    """
    if bboxes.shape[0] == 0:
        return [np.zeros((0, 6), dtype=np.float32) for _ in range(num_classes)], [np.zeros((0, 5), dtype=np.float32) for i in range(num_classes)]
    else:
        if isinstance(bboxes, torch.Tensor):
            bboxes = bboxes.detach().cpu().numpy()
            labels = labels.detach().cpu().numpy()
            uncertainties = uncertainties.detach().cpu().numpy()
        return [bboxes[labels == i, :] for i in range(num_classes)], [uncertainties[labels == i, :] for i in range(num_classes)]
...
def obb2poly_le90(rboxes):
    ...
    M = torch.stack([cos, -sin, sin, cos], dim=0).reshape(2, 2,
                                                          N).permute(2, 0, 1)
    polys = M.matmul(rects).permute(2, 1, 0).reshape(-1, N).transpose(1, 0)
    rects_flatten = rects.permute(0, 2, 1).contiguous().view(N, -1)
    polys[:, ::2] += x_ctr.unsqueeze(1)
    polys[:, 1::2] += y_ctr.unsqueeze(1)
    rects_flatten[:, ::2] += x_ctr.unsqueeze(1)
    rects_flatten[:, 1::2] += y_ctr.unsqueeze(1)
    return polys.contiguous()
