...
class DynamicAssigner(BaseAssigner):
    ...
    def __init__(self,
                 ...
                 normal_iou_thr=0.70,
                 aux_positive_thr = 0.50,
                 aux_extra_low_quality_thr = 0.01,
                 r=0.15,
                 base_size=12,
                 scale_ratio=1, 
                 neg_iou_thr=0.2,
        ...
        self.aux_positive_thr = aux_positive_thr
        self.aux_extra_low_quality_thr = aux_extra_low_quality_thr
        self.r = r
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None,
               num_base_anchors=1, scale_ratio=1.0, aux_pred_feat = None, aux_gt_feat = None):
        """Assign gt to bboxes.
        ...
        #通过bboxes和aux_pred_feats和aux_gt_feats计算有效区域
        if aux_gt_feat is not None:
            #positive_pred_indices = torch.nonzero(aux_pred_feat > self.aux_positive_thr).squeeze(1)
            positive_gt_indices = torch.nonzero(aux_gt_feat > self.aux_positive_thr).squeeze(1)
            #max_select_count = min(3 * len(positive_gt_indices), len(positive_pred_indices))
            #random_selected_indices = positive_pred_indices[torch.randperm(len(positive_pred_indices))[:max_select_count]]
            highlight = torch.zeros(aux_gt_feat.shape, dtype=torch.bool, device=aux_gt_feat.device)
            #highlight[random_selected_indices] = True
            highlight[positive_gt_indices] = True
        else:
            highlight = None
        ...
        assign_result, ign_inds = \
            self.assign_wrt_overlaps_and_aux_feat(overlaps, gt_areas, gt_labels, num_base_anchors, scale_ratio=scale_ratio, highlight=highlight)
        if assign_on_cpu:
    ...
    def assign_wrt_overlaps_and_aux_feat(self, overlaps, gt_areas, gt_labels=None, num_base_anchors=1, scale_ratio=1.0, highlight = None):
        ...
        overlaps = overlaps.view(num_gts, 1, -1)  ###候选框是(Σhw, num_base_anchors)
        ...
        _ = torch.nonzero(pos_inds)
        if highlight is not None:
            highlight = highlight & (max_overlaps >= self.aux_extra_low_quality_thr)
            _1 = torch.nonzero(highlight)
            pos_inds = pos_inds | highlight
            _2 = torch.nonzero(pos_inds)
