...
class BBoxUncHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 loss_bbox=dict(
                     type='SmoothL1Loss', beta=1.0, loss_weight=1.0),
                 init_cfg=None):
        super(BBoxUncHead, self).__init__(init_cfg)
        assert with_cls or with_reg
        self.with_avg_pool = with_avg_pool
        self.with_cls = with_cls
        self.with_reg = with_reg
    ...
    def loss(self,
        ...
        if bbox_pred_mu is not None and bbox_pred_var is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_bbox'] = bbox_pred_mu[pos_inds].sum()
        return losses
    ...
    def get_bboxes(self,
        ...
        if bbox_pred_mu is not None and bbox_pred_var is not None:
            bboxes = self.bbox_coder.decode(
                rois[..., 1:], bbox_pred_mu, max_shape=img_shape)
            #bboxes_unc = self.bbox_coder.decode_var(bbox_pred_var)
            bboxes_unc = bbox_pred_var
        else:
            bboxes = rois[:, 1:].clone()
            bboxes_unc = rois[:, 1:].clone()
            if img_shape is not None:
                bboxes[:, [0, 2]].clamp_(min=0, max=img_shape[1])
                bboxes[:, [1, 3]].clamp_(min=0, max=img_shape[0])
        ...
        if cfg is None:
            ...
            det_bboxes, det_labels, inds = multiclass_nms(bboxes, scores,
                                                    ...
                                                    cfg.max_per_img,
                                                    return_inds=True)
            #det_bboxes_unc = bboxes_unc.view(bboxes_unc.shape[0], -1, 4).reshape(-1, 4)[inds]
