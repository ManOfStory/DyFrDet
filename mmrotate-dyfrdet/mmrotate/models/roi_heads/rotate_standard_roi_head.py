...
from mmrotate.core import build_assigner, build_sampler, obb2xyxy, rbbox2result, uncrbbox2result
...
@ROTATED_HEADS.register_module()
class RotatedUncStandardRoIHead(BaseModule, metaclass=ABCMeta):
    """Simplest base rotated roi head including one bbox head.
    ...
    def __init__(self,
        ...
        super(RotatedUncStandardRoIHead, self).__init__(init_cfg)
        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        self.version = version
    ...
    def _bbox_forward(self, x, rois):
        ...
        bbox_results = dict(
            cls_score=cls_score, bbox_pred_mu=bbox_pred_mu, bbox_pred_var = bbox_pred_var, bbox_feats=bbox_feats, x_reg = x_reg)
        return bbox_results
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        det_bboxes, det_labels, det_bboxes_unc  = self.simple_test_bboxes(
            x, img_metas, proposal_list, self.test_cfg, rescale=rescale)

        bbox_results = [
            uncrbbox2result(det_bboxes[i], det_labels[i], det_bboxes_unc[i],
                         self.bbox_head.num_classes)
            for i in range(len(det_bboxes))
        ]
